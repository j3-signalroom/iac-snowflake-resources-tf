...
resource "snowflake_account_role" "role" {
  ...
  name     = upper("${var.snowflake_service_user}_ROLE")
}
...
resource "snowflake_schema" "schema" {
  ...
  name       = upper("${var.snowflake_service_user}_SCHEMA")
}
...
resource "snowflake_service_user" "user" {
  provider          = snowflake.security_admin
  name              = upper("${var.snowflake_service_user}_SERVICE_USER")
...
resource "snowflake_grant_privileges_to_account_role" "user_grant" {
  ...
  on_account_object {
    object_type = "USER"
    object_name = snowflake_service_user.user.name
  }
}
...
resource "snowflake_database" "example" {
  name = upper("${var.snowflake_service_user}_DATABASE")
}

resource "snowflake_warehouse" "example" {
  name           = upper("${var.snowflake_service_user}_WAREHOUSE")
